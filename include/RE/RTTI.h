...
namespace RE
{
	...
	namespace detail
	{
		...
		struct types_are_compat<To, From*> : std::is_pointer<To>
		{};

		template <class To, class From>
		struct types_are_compat<To, const From*> : std::conjunction<std::is_pointer<To>, std::is_const<std::remove_pointer_t<To>>>
		{};

		template <class To, class From>
		struct types_are_compat<To, volatile From*> : std::conjunction<std::is_pointer<To>, std::is_volatile<std::remove_pointer_t<To>>>
		{};

		template <class To, class From>
		struct types_are_compat<To, const volatile From*> : std::conjunction<std::is_pointer<To>, std::is_const<std::remove_pointer_t<To>>, std::is_volatile<std::remove_pointer_t<To>>>
		{};

		template <class, class = void>
		struct implements_rtti : std::false_type
		{};

		template <class T>
		struct implements_rtti<T, std::void_t<decltype(remove_cvpr_t<T>::RTTI)>> : std::true_type
		{};

		template <class To, class From>
		struct cast_is_valid : std::conjunction<types_are_compat<To, From>, target_is_valid<To>, implements_rtti<To>, implements_rtti<From>>
		{};
