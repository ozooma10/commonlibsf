...
namespace RE
{
	...
	public:
		...
		struct UserEventMapping
		{
			...
			std::uint32_t keyCode;          // 08 - virtual-key code for keyboard mappings
			std::uint32_t modifierKeyCode;  // 0C - virtual-key code for keyboard mappings
		...
		// The mapped prefix of an engine input-context object.
		struct InputContext
		{
			std::array<BSTArray<UserEventMapping>, 3> deviceMappings;  // 00 - keyboard, mouse, gamepad
		};
		...
		// These views point into engine-owned arrays. Use them only on the game thread and do not
		// retain them across a remap, which may reallocate their backing storage.
		...
		// This view is engine-owned and is valid only until the active context stack changes.
