...
namespace REL
{
	...
	std::uint64_t IDDB::offset(std::uint64_t a_id) const
	{
		...
		switch (a_id) {
		case 35706:
			address = reinterpret_cast<std::uintptr_t>(&DestroyString);
			break;
		case 124100:
			address = reinterpret_cast<std::uintptr_t>(&FormatTokens);
			break;
		case 124101:
			address = reinterpret_cast<std::uintptr_t>(&FormatCodes);
			break;
		case 124116:
			address = reinterpret_cast<std::uintptr_t>(&Load);
			break;
		case 124117:
			address = reinterpret_cast<std::uintptr_t>(&Parse);
			break;
		case 124124:
			address = reinterpret_cast<std::uintptr_t>(&Resolve);
			break;
		default:
			throw std::runtime_error("Unexpected engine relocation: " + std::to_string(a_id));
		}
